...
\begin{frame}[c,allowframebreaks]{Engineering Concurrent \& Distributed Systems (C\&DS)}
    ...
    \begin{block}{\textbf{System Engineers} must tackle \textbf{deployment}-related aspects}
        \begin{itemize}
            ...
            \item monitoring and managing remote components
            \item ensure system components work \alert{predictably} despite heterogeneity
            \item support stability despite the pace of software evolution
        \end{itemize}
    \end{block}
...
\begin{frame}[c, allowframebreaks]{About the the Lab}
    ...
    \begin{block}{Our approach: \textbf{bottom-up}}
        ...
        %
        \begin{enumerate}
            \item confident with development tools
            \item practice with basic interaction-related aspects
            \item experiment with articulated interaction protocols
            \item taste the benefits of leveraging on full-fledged middlewares
        \end{enumerate}
    \end{block}
\end{frame}
...
\begin{frame}[c,allowframebreaks]{Structure of the Lab}
    ...
    \begin{enumerate}
        ...
        %
        \begin{itemize}
            \item build automation and dependency management with Gradle
            \item containerisation and orchestration with Docker
        \end{itemize}
        ...
        %
        \begin{itemize}
            \item multi-threading and asynchronous programming, on the JVM
            \item (byte-)stream-oriented communication via Sockets, on the JVM
        \end{itemize}

        \medskip

        \item we will then present the notion of (de-)serialization
        %
        \begin{itemize}
        ...
        \item we will then present the notion of ReST API
        %
        \begin{itemize}
            \item showing how they can be formalised via Swagger
            \item discussing how web servers can be realised on the JVM, via Javalin
            \item discussing how web clients can be realised on the JVM
        \end{itemize}
        ...
        %
        \begin{itemize}
            \item building distributed middlewares, e.g. via gRPC and Protobuff
            \item uncoupling interacting entities, e.g. via RabbitMQ
            \item replicating data over a number of distributed machines, e.g. via etcd
        \end{itemize}

        \medskip

        \item finally, we will exemplify notable agent-oriented infrastructures
        %
        \begin{itemize}
...
\begin{frame}[c,allowframebreaks]{Required Technologies and Skills}
    ...
    \begin{alertblock}{Required}
        \begin{itemize}
            ...
            %
            \begin{itemize}
                \item[$\checkmark$] input/output API
...
\begin{frame}[c]{About Fora}
    \begin{itemize}
        ...
        \uurl{https://virtuale.unibo.it/mod/forum/view.php?id=611839}
        %
        \begin{itemize}
            \item don't be shy :)
...
\begin{frame}[c]{About Projects}
    \begin{itemize}
        ...
        \uurl{https://github.com/gciatto-unibo/sd-project-rules}
...
\begin{frame}[c, allowframebreaks]{Sorts of Projects}
    ...
    \begin{block}{Technology Know-How DS Project}
        ...
        \begin{itemize}
            ...
            \end{enumerate}
            \item Previous projects in this category: \ccite{Sd2021ProjectEtcd,Sd2021ProjectCEPH,Sd2021ProjectKubernetes}
